...
byte lastHourWattsMax[60];			// every minute here will be writen max Watts value.
byte lastHourWattsMax_counter =0;	// Index for lastHourWattsMax
byte lastHourWattsMaxValue = 0;	// This value is updated every minute to reflect the actual peak within last hour.
...
unsigned long loadSwitchDelay=10000;	// delay in ms for switching the load (to prevent often On/Offs).
byte loadsafety_counter = 0;	// If load switched off to often (3 times) in series during next 10 minutes after switched on, then go to standby mode
...
{
  readVoltages();
  updateMaxWattsValues(); // updates max value
  readButtons();
  ...
  if (eachMinuteTask.check() == 1) {
	...
	lastHourWattsMaxCounter_change();	// change index of array for peak Watts values
  }
...
void updateLCD() {
	...
	switch (screenNr_LCD & B01111111) {
		case 1:
		    ...
		    lcd.setCursor(10,1);
		    lcd.write(byte(2));	// temperature sign
		    lcd.print(ambient_Temperature,0);
		    lcd.write(byte(4));	// celsium sign
		    lcd.print(" ");
    		break;
    	case 2:
		    lcd.setCursor(0,0); 
		    ...
		    lcd.setCursor(0,1);
		    lcd.print("Hour peak: ");
		    lcd.print(lastHourWattsMaxValue);
		    lcd.print("W ");
    		break;
    	case 3:
		    lcd.setCursor(0,0); 
		    lcd.print("PWM: ");
		    if (pwmout==0) {lcd.print("Off ");} else {lcd.print(map(pwmout,0,1023,0,100));lcd.print("%  ");}
		    lcd.setCursor(14,0); 
		    lcd.print("S");
...
//			if (load_RELAY == LOW) {lcd.print("Off");} else {lcd.print("On ");}
		    ...
		    lcd.print("A");
		    break;
    	case 100:	//special screens (Safety - warning)
			lcd.setCursor(0,0);
			lcd.print("Safety mode!");
			lcd.setCursor(0,1);
			lcd.print("Press any button");
			screenNr_LCD = 101;	// every second switch screens 100 and 101
			break;
    	case 101:	//special screens (Safety - show Battery voltage)
			sub_updateLCD();
		    lcd.setCursor(10,0); 
		    lcd.print("SAFETY");
		    lcd.setCursor(10,1); 
			unsigned long timediff = (millis()-loadOnOffTime)/60000;	// differenece in minutes
			if (timediff<60) {	// 60 minutes
				lcd.print(timediff,0);	// in minutes
				lcd.print(" min");
			} else if (timediff<2880) {	// 48 hours
				lcd.print((timediff)/60,0);	// in hours
				lcd.print(" hrs");
			} else {	// if more than 2 days
				lcd.print((timediff)/1440,0);	// in days
				lcd.print(" day");
			}
			screenNr_LCD = 100;	// every second switch screens 100 and 101
			break;
	}
...
void changeLCDscreen() {
	if ((screenNr_LCD & B01111111) < 100) {	// skip screen changing in speacial screen.
...
// Also there is a delay for 10 seconds after load switched Off and 3 seconds after load is switched on.
// If load switched off after less than 20 minutes of last switch, and this occurs 3 times, then switch load off forever (until user intervention)
void switchLoadOnOff() {
	if(loadsafety_counter<3) {
		if (millis()-loadOnOffTime>loadSwitchDelay) {		// delay for 10 or 5 seconds
			if ((pwmout==0 && solar_currentS2_voltage<=C_CHARGEPERCENT_OPEN_0 * BATTERY_CELLS) || (pwmout>0 && solar_currentS2_voltage<=C_CHARGEPERCENT_0 * BATTERY_CELLS)) {
				if (load_RELAY != LOW) {
					load_RELAY = LOW;
					digitalWrite(BACKLIGHT_PIN, LOW);
					loadSwitchDelay = 10000;	// Delay for 10 seconds.
					// check for safety
					if (millis()-loadOnOffTime<1200000) {	// within 20 minutes
						loadsafety_counter++;
						if(loadsafety_counter>=3){
							screenNr_LCD=100;
						}
					} else {
						loadsafety_counter=0;	// normal oeration, reset safety counter
					}
					loadOnOffTime = millis();	// start a timer.
				}
				...
				if (load_RELAY != HIGH) {
					load_RELAY = HIGH;
					digitalWrite(BACKLIGHT_PIN, HIGH);
					loadSwitchDelay = 3000;	// Delay for 3 seconds.
					loadOnOffTime = millis();	// start a timer.
				}
			}
		}		
		...
		// stay in safety mode
		// check for buttons
		if (buttonReturnedValue>0) { // is any button is pressed
			loadsafety_counter=0;	// reset a safety mode
			screenNr_LCD=1;
		}
	}
}
...
// updateMaxWattsValues routine. Runs as often as possible
void updateMaxWattsValues() {
	// store max value to the current index
	if (lastHourWattsMax[lastHourWattsMax_counter]<(solar_power / 1000)) {
		lastHourWattsMax[lastHourWattsMax_counter]=solar_power / 1000;
	}
}

// change index of lastHourWattsMax. Runs every minute
void lastHourWattsMaxCounter_change() {
	lastHourWattsMax_counter++;
	if (lastHourWattsMax_counter==60) {lastHourWattsMax_counter=0;}
	lastHourWattsMax[lastHourWattsMax_counter]=0;	// reset current value to update max
	//Serial.print("LastHourCounter: ");Serial.println(lastHourWattsMax_counter);
	// update lastHourWattsMaxValue variable, to reflect Peak within last hour.
	lastHourWattsMaxValue=0;
	for (byte foo = 0; foo < 60; foo++)  {
		if (lastHourWattsMax[foo]>lastHourWattsMaxValue) {
			lastHourWattsMaxValue=lastHourWattsMax[foo];
		}
	}	
}
